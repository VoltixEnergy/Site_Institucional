...
function autenticar(email, senha) {
    ...
    var instrucaoSql = `
        SELECT id_usuario, nome, email, fk_empresa FROM usuario WHERE email = '${email}' AND senha = '${senha}';
    `;
...
function cadastrar(nome, email, senha, cpf, nivelPermissao = 1, fkEmpresa) {
    ...
    var instrucaoSql = `
        INSERT INTO usuario (nome, email, senha, cpf, nivel_permissao, fk_empresa) VALUES ('${nome}', '${email}', '${senha}', '${cpf}', '${nivelPermissao}', '${fkEmpresa}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
